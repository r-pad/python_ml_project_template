...
${PARTITION_OPTION}
#SBATCH --job-name=./.slurm_logs/${JOB_NAME}
...
for ((i = 1; i <= worker_num; i++)); do
  ...
  srun --nodes=1 --ntasks=1 -w "$node_i" \
    bash -c "CUDA_VISIBLE_DEVICES=$cuda_devices ray start --address "$ip_head" --redis-password="$redis_password" --block --num-gpus=$num_gpus" &
  sleep 5
done
